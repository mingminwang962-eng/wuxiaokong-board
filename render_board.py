...
if(policy.standardFlow?.length){h+=`<section class="policy"><div class="policy-head"><strong>交付、复核与合并路径</strong><span class="gate ${policy.remotePushStatus==='HOLD'?'HOLD':'PASS'}">远端推送 ${esc(policy.remotePushStatus||'—')}</span></div><ol>${policy.standardFlow.map(x=>`<li>${esc(x)}</li>`).join('')}</ol><p class="prep-detail">统一接收分支：<b>${esc(policy.targetBranch||'未定')}</b></p>${policy.temporaryException?`<p class="policy-note"><b>当前说明：</b>${esc(policy.temporaryException)}</p>`:''}${policy.protectionNote?`<p class="prep-detail" style="margin-top:9px">${esc(policy.protectionNote)}</p>`:''}</section>`;}
